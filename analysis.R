...
black_ny_plot <- ggplot(data = ny_black_jail, mapping = aes(
  x = year,
  y = black_jail_pop
)) +
...
white_ny_plot <- ggplot(data = ny_white_jail, mapping = aes(
  x = year,
  y = white_jail_pop
)) +
  geom_bar(mapping = aes(fill = year), stat = "identity") +
...
test <- ggplot(data = nytest, mapping = aes(
  x = black_jail_pop, y = white_jail_pop
)) +
  geom_point(color = "blue") +
  labs(x = "Black Jail Population",
       y = "White Jail Population",
       title = "Black vs. White Imprisonment in NY")
